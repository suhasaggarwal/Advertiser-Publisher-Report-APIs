...
public class AddChannelCampaignDAO1 {
	...
	// Campaign data will be loaded from form and individual fields will be
	// populated

	public static String AddChannelCampaignData(ChannelCampaign campaign,
			String emailid) {
		...
		try {
			...
			if (campaign.getOptimumParameter() != null
					&& campaign.getCampId() != null) {

				status1 = IdealTargetingParametersUpdate
						.setIdealTargetingParameters(campaign, emailid);
			}
				...
				List<OperatingSystem> OperatingSystemId = campaign
						.getOperatingsystemid();
				String operatingsystemid = "";

				if (OperatingSystemId != null
						&& OperatingSystemId.isEmpty() == false) {
					for (int i = 0; i < OperatingSystemId.size(); i++) {

						operatingsystemid = operatingsystemid
								+ OperatingSystemId.get(i).getId() + ",";
				...
				List<AudienceSegment> InterestSegmentId = campaign
						.getInterestsegmentid();
				;

				String interestsegmentid = "";

				if (InterestSegmentId != null
						&& InterestSegmentId.isEmpty() == false) {
					for (int i = 0; i < InterestSegmentId.size(); i++) {

						interestsegmentid = interestsegmentid
								+ InterestSegmentId.get(i).getParent_id() + ":"
								+ InterestSegmentId.get(i).getId() + ",";
				...
				/*
				 * query1 =
				 * "INSERT INTO Campaign(CampaignName,ObjectiveId,StartDate,EndDate,Status,Approval,Email) VALUES("
				 * +
				 * "'"+campName+"'"+","+"'"+ObjectiveId+"'"+","+"'"+startDate+"'"
				 * +
				 * ","+"'"+endDate+"'"+","+"'"+status+"'"+","+"'"+Approval+"'"+","
				 * +"'"+Email+"'"+")";
				 * 
				 * try { stmt.executeUpdate(query1); } catch (SQLException e1) {
				 * // TODO Auto-generated catch block e1.printStackTrace(); }
				 * 
				 * 
				 * try { rs = stmt.executeQuery(
				 * "select last_insert_id() as last_id from Campaign"); } catch
				 * (SQLException e1) { // TODO Auto-generated catch block
				 * e1.printStackTrace(); } try { campId = rs.getInt("last_id");
				 * } catch (SQLException e) { // TODO Auto-generated catch block
				 * e.printStackTrace(); }
				 */

				query2 = "INSERT INTO BudgetCampaignChannel(BudgetPercentage,ChannelBudget,CampaignTypeId,MonthlyCap,DailyCap,MaxBid,ExpectedBid,ClickTracker,Comments)  VALUES("
						+ "'"
						+ BudgetPercentage
						+ "'"
						+ ","
				...
				try {
					rs = stmt
							.executeQuery("select last_insert_id() as last_id from BudgetCampaign");
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				...
				query4 = "INSERT INTO TargetCampaign(TargetType,Lookalike,InterestSegmentId,GeographyId,AgeRange,Gender,IncomeRange) VALUES("
						+ "'"
						+ TargetType
						+ "'"
						+ ","
				...
				try {
					rs = stmt
							.executeQuery("select last_insert_id() as last_id1 from TargetCampaign");
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				...
				query5 = "INSERT INTO TargetCampaignDevice(TargetCampaignId,DeviceId) VALUES("
						+ "'" + lastid1 + "'" + "," + "'" + deviceid + "' )";
				...
				query6 = "INSERT INTO TargetCampaignOS(TargetCampaignId,OperatingSystemId) VALUES("
						+ "'"
						+ lastid1
						+ "'"
						+ ","
				...
				query7 = "INSERT INTO TargetCampaignResolution(TargetCampaignId,ResolutionId) VALUES("
						+ "'"
						+ lastid1
						+ "'"
						+ ","
				...
				/*
				 * query8 =
				 * "INSERT INTO CampaignDetail(CampaignId,BudgetCampaignId,TargetCampaignId) VALUES("
				 * +"'"+campId+"'"+","+"'"+lastid+"'"+","+"'"+lastid1+"' )";
				 * 
				 * try { stmt.executeUpdate(query8); } catch (SQLException e1) {
				 * // TODO Auto-generated catch block e1.printStackTrace(); }
				 */
				query9 = "INSERT INTO CampaignChannel(CampaignId,CreativeId,ChannelId,Status,EmailId,StartDate,EndDate,BudgetCampaignChannelId,TargetCampaignChannelId) VALUES("
						+ "'"
						+ Id
						+ "'"
						+ ","
