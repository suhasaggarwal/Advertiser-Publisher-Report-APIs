	...
	public static List<Reports> FilterReportDTO(List<Reports> Report,
		...
		//  List<Str>
	      ...
	      for(int i=0; i<Report.size(); i++){
	    	  ...
	    	  if(campDateMap.containsKey(campaignId)==false){
	    	  dates = new ArrayList<String>();
	    	  dates.add(date);
	    	  campDateMap.put(campaignId,dates);
	    	  finalDTO.add(Report.get(i));
	    	  }
	    	  else{
	    		dates = new ArrayList<String>();
	    		dates = campDateMap.get(campaignId);  
...
//	 return Report;
	...
	public static List<Reports> FilterChannelDTO(List<Reports> Report,
			String dateStart, String dateEnd, int reportCode) {
			
		  List<Reports> finalDTO = new ArrayList<Reports>();
          Map<String,List<String>> channelDateMap = new HashMap<String,List<String>>();
		  String channel = null;
		  String date;
		//  List<Str>
		  List <String> dates=null;
	      ...
	      for(int i=0; i<Report.size(); i++){
	    	...
	    	channel = Report.get(i).getChannel();
	    	  ...
	    	  if(channelDateMap.containsKey(channel)==false){
	    	  dates = new ArrayList<String>();
	    	  dates.add(date);
	    	  channelDateMap.put(channel,dates);
	    	  finalDTO.add(Report.get(i));
	    	  }
	    	  else{
	    		dates = new ArrayList<String>();
	    		dates = channelDateMap.get(channel);  
	    		if(dates.contains(date)==true){
//	    			Report.remove(i);
	    		}
